...
static void nk_gdi_render_on_dc(nk_gdi_ctx gdi, struct nk_color clear, HDC memory_dc);
    ...
    struct nk_image img, struct nk_color col)
{
    ...
    if (!gdi->window_dc || !hbm)
        return;

    hDCBits = CreateCompatibleDC(gdi->window_dc);
    GetObject(hbm, sizeof(BITMAP), (LPSTR)&bitmap);
    SelectObject(hDCBits, hbm);
    StretchBlt(gdi->window_dc, x, y, w, h, hDCBits, 0, 0, bitmap.bmWidth, bitmap.bmHeight, SRCCOPY);
    DeleteDC(hDCBits);
}
...
#ifdef NK_GDI_DRAW_ON_SCREEN
        nk_gdi_render_on_dc(gdi, nk_color(0, 0, 0, 0), dc);
#else
        nk_gdi_blit(gdi, dc);
#endif
        EndPaint(wnd, &paint);
...
{
    nk_gdi_render_on_dc(gdi, clear, gdi->memory_dc);
