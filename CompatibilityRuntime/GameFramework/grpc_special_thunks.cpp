...
void gprsharp_convert_clock_type_raw_thunk(void) {
    ...
    storeX86CallResult(context, gprsharp_convert_clock_type(input, input_clock));
}
...
void grpcsharp_channel_create_call_raw_thunk(void) {
    ...
    storeX86CallResult(context, grpcsharp_channel_create_call(channel, parent_call, propagation_mask, cq, method, host, deadline));
}
