...
const ReplacementExport *findEntryInReplacementExports(const char *name, const ReplacementExport *rebindTo, size_t numberOfReboundExports) {
...
void rebindModuleImport(HMODULE importingModule, const IMAGE_IMPORT_DESCRIPTOR *importedModule,
    ...
    for(; *importLookupTable && *importAddressTable; importLookupTable++, importAddressTable++) {
        ...
        if(resolved->originalAddress) {
            *resolved->originalAddress = reinterpret_cast<void *>(*importAddressTable);
        }

        *importAddressTable = reinterpret_cast<uint64_t>(resolved->address);
