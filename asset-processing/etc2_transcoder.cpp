...
[[noreturn]] static void assertionFailed() {
    throw std::logic_error("assertion failed");
}

#define UNREACHABLE() assertionFailed()
#define ASSERT(expr) do { if(!(expr)) { assertionFailed(); } } while(0)
...
struct ETC2Block
{
  ...
  private:
    ...
    static T renormalizeEAC(int value)
    {
        ...
        return static_cast<T>(std::clamp(value, lower, upper)) << shift;
    }
...
void LoadETC2RGBA8ToBC3(size_t width,
                        ...
                        bool isSigned)
{
    printf("LoadETC2RGBA8ToBC3: width %zu height %zu depth %zu input %p input row pitch %zu input depth pitch %zu output %p output row pitch %zu output depth pitch %zu\n",
           width, height, depth, input, inputRowPitch, inputDepthPitch, output, outputRowPitch, outputDepthPitch);
...
void LoadETC2RGBA8ToRGBA8(size_t width,
                          ...
                          size_t outputDepthPitch)
{
    LoadETC2RGBA8ToRGBA8(width, height, depth, input, inputRowPitch, inputDepthPitch,
                         output, outputRowPitch, outputDepthPitch, false);
}
...
void LoadETC2RGBA8ToBC3(size_t width,
                        ...
                        size_t outputDepthPitch)
{
    LoadETC2RGBA8ToBC3(width, height, depth, input, inputRowPitch, inputDepthPitch, output,
                       outputRowPitch, outputDepthPitch, false, false);
}
...
void LoadEACR11ToBC4(size_t width,
                     ...
                     size_t outputDepthPitch)
{
    LoadEACR11ToBC4(width, height, depth, input, inputRowPitch, inputDepthPitch, output,
                    outputRowPitch, outputDepthPitch, false);
}
...
void LoadEACRG11ToBC5(size_t width,
                      ...
                      size_t outputDepthPitch)
{
    LoadEACRG11ToBC5(width, height, depth, input, inputRowPitch, inputDepthPitch, output,
                     outputRowPitch, outputDepthPitch, false);
}
